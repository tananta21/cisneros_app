...
@section('menu_modulos')
    ...
    <h3 class="col-lg-12" style="margin-bottom: 0.5rem">Gestor de Modelos</h3>
    <hr class="col-lg-12 linea-titulo" size="5px" color="green"/>

    <div class="col-lg-7">
        {!! Form::model(Request::all(),['route'=>'buscar.modelo','method' => 'get', 'class' => 'form-horizontal', 'role'=>'form']) !!}
        <div class="col-lg-2">
            <span style="font-size: 1.5rem">Estado Modelo</span>
        </div>
        <div class="col-lg-4">
            {!!form::select('estado',[
            '1'=>'Activo',
    ...
    <div class="col-lg-5">
        <a  onclick="buscarMarcas()" href="#" style="font-size: 1.6rem">
            <i class="fa fa-plus-circle fa-2x" aria-hidden="true"></i> <span style="color: #000000">Agregar Nuevo Modelo</span>
        </a>
    </div>
            ...
            <table id="marca" class=" table table-hover display" cellspacing="0" width="100%">
                ...
                @foreach($marcas as $categoria)
                    <tr data-id="{{$categoria->id}}" id="filaproducto{{$categoria->id}}">
                        <td>{{$categoria->id}}</td>
                        <td>{{$categoria->marca->descripcion}}</td>
                        ...
                        <td>
                            @if($categoria->estado == 1)
                                ...
                                <a id="editar_marca{{$categoria->id}}" onclick="editarCategoria('{{$categoria->id}}')" style="cursor:pointer; color: green;  font-size: 2.5rem; padding: 0.5rem">
                                    <input type="hidden" name="editarmarca{{$categoria->id}}" value="{{$categoria->id}}"/>
